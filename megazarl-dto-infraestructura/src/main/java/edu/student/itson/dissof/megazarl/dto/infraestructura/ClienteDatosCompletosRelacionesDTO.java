...
public class ClienteDatosCompletosRelacionesDTO extends ClienteDTO{
    ...
    public IdDireccionDTO getIdDireccionEnvio() {
        return new IdDireccionDTO(direccionEnvio.getId());
    }

    @Override
    public List<IdCarritoComprasDTO> getIdsCarritosCompras() {
        
        List<IdCarritoComprasDTO> idsCarritosCompras = new LinkedList<>();
        
        for(CarritoComprasDTO carritoComprasDTO: carritosCompras){
            idsCarritosCompras.add(new IdCarritoComprasDTO(carritoComprasDTO.getId()));
        }
